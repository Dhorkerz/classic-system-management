...
//db connection
	...
	//select db
		...
		if(isset($_POST['Payment'])){
			...
			//perform query
			$query="INSERT INTO payment(studentid,coursecode,amount_payable,amount_paid,balance,received_by,payment_date)
			VALUES('$studentid','$coursecode','$amountpayable','$amountpaid','$balance','$receivedby','$paymentdate')";
			$query=mysql_query($query);
			if (!$query) {
				die(mysql_error());
			}
			else{
				/*query result which redirects user to the payment dashboard*/
				header('location:csmPaymentDashboard.php?msg= payment Added Successfully');
			}
		}
	}
?>
...
<body>
	...
	<div class="container-fluid" id="wrapper">
		<div id="aside"> 
			...
			<a href="csmPayment.php" class="col-12 active" id="link">Payment History</a>
		...
		<div id="main">	
			...
			<div id="content">
				...
				<form method="POST" action="csmPayment.php">

						<div class="form-group">
						<label >STUDENT ID:</label>
						<input class="form-control" type="text" name="studentid" value="<?php
							echo $_POST["studentid"]; ?>"><br>
							<?php
							echo validateInput($_POST["studentid"]);
							?>
						</div>
						...
						<input class="form-control" type="text" name="amountpayable" value="<?php
							echo $_POST["amountpayable"]; ?>">
						<?php
						echo validateInput($_POST["amountpayable"]);?>
						</div>

						<div class="form-group">
						<label>AMOUNT PAID:</label>
						<input class="form-control" type="text" name="amountpaid" value="<?php
							echo $_POST["amountpaid"]; ?>">
							<?php
							echo validateInput($_POST["amountpaid"]);?>
						</div>

						<div class="form-group">
						<label>BALANCE:</label>
						<input class="form-control" type="text" name="balance" value="<?php
							echo $_POST["balance"]; ?>">
							<?php
							echo validateInput($_POST["balance"]);?>
						</div>

						<div class="form-group">
						<label>RECEIVED BY:</label>
						<input class="form-control" type="text" name="receivedby" value="<?php
							echo $_POST["receivedby"]; ?>">
							<?php
							echo validateInput($_POST["receivedby"]);?>
						</div>

						<div class="form-group">
						<label>PAYMENT DATE:</label>
						<input class="form-control" type="Date" name="paymentdate" value="<?php
							echo $_POST["paymentdate"]; ?>">
							<?php
							echo validateInput($_POST["paymentdate"]);?>
							</div>

						<input class="btn btn-primary" type="submit" name="Payment" value="PAYMENTS">
